...
| 图数据库 | Neo4j 5 Community（首年；第 2 年评估 Enterprise，基准 §6.1） |

> **V1.2 修订说明**：依据 2026-07-27 文档-代码一致性审计修订，对齐代码库实际状态（backend/app/models/*.py、backend/alembic/versions/0001_initial.py）与 FRD-BASELINE-V1.1。本版对 §3 全部表定义按基准 §3 枚举字典逐条对账，修复 V1.1"变更记录声称已统一、正文未落地"的系统性偏差；代码侧尚未实现的项均以"迁移 0002 / Mx"标注现状差距，不删除目标设计。
...
> **现状说明（2026-07-27 审计）**：当前代码主键以 `uuid4` 占位（backend/app/db/base.py PKMixin，已标 TODO），UUID v7（RFC 9562，时间戳有序保证索引局部性）实现列入 M4。
...
| channel | VARCHAR(20) | 是 | - | 渠道：WEB/APP/POS/API/QR（基准 §3.8，V1.2 修正：ATM→QR） |
...
| user_created_at | TIMESTAMPTZ | 是 | - | 用户注册时间（账户年龄特征；**迁移 0002 待新增**，MVP 过渡期暂存 metadata） |
| acquirer_id | VARCHAR(50) | 是 | - | 收单机构（**迁移 0002 待新增**，过渡期暂存 metadata） |
| shipping_country | VARCHAR(2) | 是 | - | 货运地址国别（**迁移 0002 待新增**，过渡期暂存 metadata） |
| billing_country | VARCHAR(2) | 是 | - | 账单地址国别（**迁移 0002 待新增**，过渡期暂存 metadata） |
...
> **V1.2 现状说明（2026-07-27 审计）**：
> 1. **迁移 0002 待新增字段**：`user_created_at` / `acquirer_id` / `shipping_country` / `billing_country` 已在 D05 §4.1 API 声明并由 schema 接收，0001 迁移未建列；MVP 过渡期暂存 `transactions.metadata` JSONB，迁移 0002 落列并回填（另：`ip_geo` 同走 metadata）。
> 2. **MVP 不分区**：0001 迁移为普通表（未创建分区与 hypertable）；TimescaleDB 扩展已在 infra/postgres/init 安装。数据量达标后启用分区与 hypertable，列入迁移 0002+。
...
| risk_score | DECIMAL(5,4) | 否 | - | 风险分 0.0000-1.0000（基准 §3.5/§4.2，V1.2 修正：INT 0-100 → DECIMAL(5,4)） |
| risk_band | VARCHAR(10) | 否 | - | 风险等级：LOW/MEDIUM/HIGH/CRITICAL（基准 §3.5，V1.2 补列） |
...
| level | VARCHAR(10) | 否 | - | 等级：P0/P1/P2/P3（基准 §3.7，V1.2 补 P3） |
...
| status | VARCHAR(20) | 否 | 'PENDING' | 状态：PENDING/APPROVED/REJECTED/WITHDRAWN（基准 §3.10，V1.2 补 WITHDRAWN） |
...
| severity | VARCHAR(10) | 否 | 'WARN' | 严重度：INFO/WARN/BLOCK（对齐 D05 §5.1；**迁移 0002 待新增**，V1.2 补列） |
...
| metric_type | VARCHAR(10) | 否 | - | 指标：PSI/KL/KS/WASSERSTEIN（基准 §3.12，V1.2 补 KS/WASSERSTEIN） |
...
| status | VARCHAR(20) | 否 | 'PENDING' | 状态：PENDING/SUBMITTED/ACCEPTED/REJECTED（基准 §3.9，V1.2 修正：原 PENDING/REVIEWED/SUBMITTED 与基准不符） |
...
| consent_type | VARCHAR(30) | 否 | - | 同意类型：EXPLICIT/IMPLICIT_BY_ACTION（V1.2 统一为 D05 §13.1 口径；原 DATA_PROCESSING/MARKETING/THIRD_PARTY_SHARE 与 D05 矛盾） |
| consent_status | VARCHAR(20) | 否 | - | 状态：GRANTED/WITHDRAWN/EXPIRED（基准 §3.11，V1.2 补 EXPIRED） |
...
| status | VARCHAR(20) | 否 | 'PENDING' | 状态：PENDING_REVIEW/APPROVED/IN_PROGRESS/COMPLETED/REJECTED/PARTIALLY_COMPLETED（V1.2 对齐 D05 §13.6；原 PENDING/PROCESSING/COMPLETED/REJECTED 与 D05 不符） |
...
> V1.2 修正：改 0-1 四档刻度，对齐基准 §3.5 与 scores.risk_score DECIMAL(5,4)（V1.1 为 0-100 旧刻度，与基准矛盾）。

| 等级 | 数值范围 | 决策 | 颜色 |
|---|---|---|---|
| LOW | < 0.30 | ALLOW | 绿色 |
| MEDIUM | 0.30-0.60 | REVIEW | 蓝色 |
| HIGH | 0.60-0.85 | REVIEW | 橙色 |
| CRITICAL | ≥ 0.85 | DENY | 红色 |

> 注：决策列为常规映射；CHALLENGE 由命中挑战类规则触发（D05 §4.1），不单纯由分数区间决定。
...
| GRANTED | 已授予 |
| WITHDRAWN | 已撤回 |
| EXPIRED | 已过期（基准 §3.11，V1.2 补） |

### 4.8 删除请求状态

> V1.2 对齐 D05 §13.6 状态机。
...
> **V1.2 现状说明（0001 迁移）**：0001 迁移索引名为 SQLAlchemy 默认 `ix_*` 前缀（非本节 `idx_*`/`uk_*` 规范），且本清单中部分索引尚未创建（`idx_transactions_card_token_occurred_at`、`idx_transactions_merchant_occurred_at`、`idx_transactions_parent_tx_id`、`idx_scores_model_version`、`idx_scores_decision_scored_at`、`idx_cases_assigned_to`、`idx_cases_chargeback_id` 等）；V1.1 变更记录所称"28 处索引重命名"未在 0001 迁移落地。命名对齐与缺失索引补齐列入迁移 0002。

### 5.2 分区策略

> **V1.2 现状说明**：MVP 阶段 0001 迁移全部普通表（未分区）；下表为目标策略，启用时点列入迁移 0002+（数据量达标后）。
...
`transactions` 表使用 TimescaleDB hypertable（目标设计，**迁移 0002+ 启用**，MVP 为普通表）：
...
- 命名：`{YYYYMMDD}_{HHMM}_{slug}.py`（注：初始迁移 `0001_initial.py` 为例外存量，自 0002 起按本规范命名）
...
> **V1.2 现状说明（0001 迁移）**：0001 迁移对 tenant_id 可空表统一使用策略名 `tenant_isolation`（USING/WITH CHECK 均含 `tenant_id IS NULL` 判断，语义与本节一致），与本节 `tenant_or_global` 命名存在差异；策略更名列入迁移 0002。
