...
### 3.0.1 当前实现对照表（截至 2026-07-27）

> 本表依据 2026-07-27 文档-代码一致性审计编制，反映代码库实际开发状态。M1-M8 计划日期不变，此表仅标注当前进展。状态图例：✅ 已实现 ｜ 🔨 骨架/部分 ｜ ⏳ 未启动

| FR 模块 | 需求编号 | 计划交付 | 当前状态 | 说明 |
|---|---|---|---|---|
| 认证权限 | FR-AUTH-001~006 | Alpha(M4) | ⏳ 未启动 | auth.py 返回 mock token；无 users 表/模型；无 TOTP 2FA |
| 商户接入 | FR-MERCHANT-001~004 | Alpha(M4) | ⏳ 未启动 | merchants/api_keys 表已建；无 /merchants 路由；webhooks.py 全 stub |
| 同步评分 | FR-SCORE-001,004,006,008 | Alpha(M4) | 🔨 骨架 | 路由注册完成；双轨融合编排器 + Kill Switch + 限流已实现；Idempotency-Key 幂等未实现 |
| SHAP 解释 | FR-SCORE-005 | Alpha(M4) | 🔨 骨架 | 异步 SHAP（ADR-007），响应含 shap_status=PENDING + task_id；P99 不含 SHAP 计算 |
| 异步/批量评分 | FR-SCORE-002,003 | Beta(M5) | ⏳ 未启动 | score/async、score/batch、tasks/{task_id} 全 TODO stub |
| 4 层回退 | FR-SCORE-007 | Beta(M5) | 🔨 部分 | Kill Switch→启发式 + 模态级熔断已实现；无备用 ML 层；同卡缓存 TTL 24h（与 5min 目标不符） |
| 结构化特征 | FR-STRUCT-001~005,003a/b | Beta(M5) | 🔨 训练侧 | ml/training 训练管线真实；**在线推理为金额启发式 Mock**；设备指纹 SDK 无代码 |
| 规则引擎 | FR-RULE-001,003~005 | Alpha(M4) | ⏳ 未启动 | rules.py 全 stub；rule_engine.evaluate 恒返回 ALLOW 空命中；DSL 选型待 ADR 锁定 |
| 预警/案件 | FR-WARN-001~006, FR-CASE-001~006 | Alpha~Beta | ⏳ 未启动 | cases.py 全 stub；workers generate_case 为 TODO；cases/case_events/appeals 表已建 |
| GNN 团伙 | FR-GNN-001,002,004 | Beta(M5) | 🔨 模块级 | gnn/ 模块代码真实（GraphSAGE+Louvain）；api/v1/gnn.py 全 stub |
| 模型治理 | FR-MODEL-001~006 | Beta(M5) | 🔨 部分 | Kill Switch service 已实现（4 级状态机）；无 /governance 管理 API；ml/drift 代码真实；workers drift_check TODO |
| AML 反洗钱 | FR-AML-001~006 | Beta(M5) | ⏳ 未启动 | aml_reports/sanction_screenings 表已建；无 /aml 路由、无 AML service |
| 报表中心 | FR-REPORT-001~005 | Beta(M5) | ⏳ 未启动 | 无 /reports 路由；前端无报表页 |
| 操作审计 | FR-AUDIT-001,002,007 | Beta(M5) | 🔨 部分 | AuditMiddleware 仅 structlog 输出（不写库/无哈希链）；无 /audit-logs 查询 API |
| 系统管理 | FR-ADMIN-001~005 | Beta(M5) | ⏳ 未启动 | 无 users 表；无 /tenants、/users、/alerts 路由（tenants 表已建） |
| PIPL 权利 | FR-PIPL-001~005 | Beta(M5) | 🔨 接口面 | pipl.py 8 端点齐备（含 rectification）；handler 全 TODO |
| 名单管理 | FR-LIST-001,002,004 | Beta(M5) | ⏳ 未启动 | 仅 sanction_screenings 表；无名单 API、无命中逻辑 |
| 计费 | FR-BILLING-001,002,004 | Beta(M5) | ⏳ 未启动 | 无任何计费代码（无表/API/worker） |

**统计**：已实现 0 ｜ 骨架/部分 8 ｜ 未启动 10

**已知问题**：
- 前端 audit.ts 调用 `/audit-logs`、model.ts 调用 `/governance/kill-switch`——两个端点后端不存在（404）
- DashboardView KPI 引用不存在的 `/reports/summary`
...
- Given 回调失败
- When 重试 5 次（退避 即时/1m/5m/30m/2h/12h，与 D05 §11.10 及代码 webhook.py 一致）
- Then 最终失败记入死信队列 + 告警
...
- Given 模型推理完成
- When 返回评分结果
- Then risk_score ∈ [0.0000, 1.0000]（DECIMAL(5,4)）+ risk_band ∈ {LOW, MEDIUM, HIGH, CRITICAL} + shap_status=PENDING + shap_task_id（SHAP Top5 **异步返回**，ADR-007；P99 不含 SHAP 计算）
...
| V1.2 | 2026-07-28 | 依据文档-代码一致性审计修订：新增 §3.0.1 实现对照表（18模块逐条标注）；SHAP 改异步口径（ADR-007）；webhook 重试改 1m/5m/30m/2h/12h（对齐 D05/代码） |
