...
| 消息队列 | Kafka 3.6 | RabbitMQ | 高吞吐 + 流处理 + DB 异步消费 |
...
| Stream Processor | Kafka Consumer | 实时流处理 + PostgreSQL 异步写入（ADR-014）|
| Database | PostgreSQL + TimescaleDB | 业务数据 + 时序（启用 RLS，ADR-015）|
| Graph DB | Neo4j 5 Enterprise | 账户-商户-设备关系图 |
| Cache | Redis Cluster | 缓存 + pubsub + 限流 + 模态历史分数 |
| Message Queue | Kafka 3.6 | 交易流 + 事件流 + 审计流 |
...
处理（三模态并行，asyncio.gather，ADR-011）:
  ...
  步骤拆解:
  1. structured → LightGBM → score_struct                  (P99 20ms)
...
| **L2 模型级 Kill Switch** | 单一 ML 模型（如 qwen-max / LightGBM-structured） | 单模型 AUC 跌破阈值、PSI > 0.25、推理成功率 < 99% | 仅该模型评分跳过，其他模型正常 | 备用 ML 模型（ADR-010 第 2 层） |
